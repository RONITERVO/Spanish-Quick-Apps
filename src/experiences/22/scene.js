...
export function mountScene() {
  const canvas = document.getElementById("cosmic-contact-spectrum");
  ...
  function sceneUnresolved(z, time) {
    const s = Math.min(width, height);
    for (let i = 0; i < 95; i++) {
      const x = seeded(i + 401) * width,
        y = seeded(i + 701) * height,
        r = 0.4 + seeded(i + 901) * 2;
      circle(
        x,
        y,
        r,
        rgba(i % 8 ? z.color : [255, 225, 170], 0.1 + 0.25 * seeded(i + 1201)),
      );
    }
    drawSignal(width * 0.5, height * 0.38, s * 0.16, z, time, 9);
    drawWaveform(
      width * 0.12,
      height * 0.63,
      width * 0.76,
    ...
    for (let i = 0; i < 6; i++) {
      const a = (i / 6) * TAU + time * 0.012;
      circle(
        width * 0.5 + Math.cos(a) * s * 0.25,
        height * 0.38 + Math.sin(a) * s * 0.15,
        2 + (i % 2),
        rgba(z.color, 0.42),
      );
    }
    footerLabel("ANOMALÍA ≠ CONCLUSIÓN", z);
  }
  function sceneDetection(z, time) {
    const s = Math.min(width, height);
    drawDish(width * 0.23, height * 0.68, s * 0.23, z, -0.08, true);
    drawDish(width * 0.76, height * 0.72, s * 0.18, z, 0.12, true);
    drawWaveform(
      width * 0.2,
      height * 0.18,
      width * 0.62,
    ...
    for (let i = 0; i < 6; i++) {
      const x = width * (0.16 + i * 0.135);
      line(x, height * 0.52, x, height * 0.58, rgba(z.color, 0.35), 1.2);
      circle(x, height * 0.5, 3 + (i % 2), rgba([255, 240, 190], 0.55));
    }
    footerLabel("MEDIR TAMBIÉN SIGNIFICA AUDITAR EL INSTRUMENTO", z);
  }
  function sceneInterference(z, time) {
    const s = Math.min(width, height);
    drawDish(width * 0.5, height * 0.72, s * 0.23, z, 0, true);
    for (let i = 0; i < 4; i++) {
      const x = width * (0.12 + i * 0.25),
        y = height * (0.22 + 0.08 * (i % 2));
      drawSatellite(x, y, s * 0.18, z, time * 0.03 + i);
      ctx.setLineDash([4, 7]);
      line(
        x,
        y,
        ...
        height * 0.61,
        rgba(
          i === Math.floor(pointerX * 4) ? [255, 225, 170] : z.color,
          i === Math.floor(pointerX * 4) ? 0.55 : 0.14,
        ),
        1.4,
      );
      ctx.setLineDash([]);
    ...
    footerLabel("CERCA, REFLEJADO, DIGITAL O CELESTE", z);
  }
  function sceneVerification(z, time) {
    const s = Math.min(width, height),
      sourceX = width * 0.5,
      sourceY = height * 0.22;
    drawSignal(sourceX, sourceY, s * 0.1, z, time, 6);
    const sites = [
      [0.15, 0.72],
      [0.36, 0.78],
      [0.64, 0.78],
    ...
    footerLabel("ERRORES DISTINTOS · RESULTADO COMPATIBLE", z);
  }
  function sceneOrigin(z, time) {
    const s = Math.min(width, height);
    drawStar(width * 0.72, height * 0.25, s * 0.055, [255, 220, 130], time);
    drawSignal(width * 0.72, height * 0.25, s * 0.12, z, time, 7);
    drawDish(width * 0.2, height * 0.73, s * 0.2, z, -0.05, true);
    ctx.setLineDash([5, 8]);
    line(
      width * 0.25,
      height * 0.63,
    ...
    ctx.setLineDash([]);
    for (let i = 0; i < 12; i++) {
      const p = fract(time * 0.045 + i / 12),
        x = mix(width * 0.68, width * 0.25, p),
        y = mix(height * 0.29, height * 0.63, p);
      circle(x, y, 1.6 + (i % 2), rgba([255, 245, 205], 0.55 * (1 - p * 0.3)));
    }
    drawWaveform(
      width * 0.4,
      height * 0.61,
    ...
    footerLabel("DISTANTE NO SIGNIFICA ARTIFICIAL", z);
  }
  function scenePattern(z, time) {
    const s = Math.min(width, height);
    const primes = [2, 3, 5, 7, 11, 13, 17, 19];
    for (let i = 0; i < primes.length; i++) {
      const x = width * (0.09 + i * 0.115),
        h = s * (0.03 + (primes[i] / 19) * 0.2);
      line(x, height * 0.7, x, height * 0.7 - h, rgba(z.color, 0.58), 2);
      circle(x, height * 0.7 - h, 3, rgba([255, 240, 180], 0.75));
      ctx.font = `900 ${s * 0.018}px system-ui`;
      ctx.textAlign = "center";
      ctx.fillStyle = "rgba(255,255,255,.55)";
      ctx.fillText(String(primes[i]), x, height * 0.76);
    }
    drawWaveform(
      width * 0.14,
      height * 0.19,
    ...
    footerLabel("ORDEN FUNCIONAL · ORIGEN TODAVÍA ABIERTO", z);
  }
  function sceneEvidence(z, time) {
    const s = Math.min(width, height);
    const levels = [
      "INTERESANTE",
      "INEXPLICADA",
    ...
    for (let i = 0; i < levels.length; i++) {
      const y = height * (0.72 - i * 0.115),
        w = width * (0.28 + i * 0.1),
        x = (width - w) / 2;
      ctx.fillStyle = rgba(z.color, 0.045 + i * 0.025);
      ctx.strokeStyle = rgba(z.color, 0.18 + i * 0.1);
      roundedRectPath(x, y, w, s * 0.07, s * 0.02);
      ctx.fill();
      ctx.stroke();
      ctx.font = `900 ${s * 0.014}px system-ui`;
      ctx.textAlign = "center";
      ctx.fillStyle = `rgba(255,255,255,${0.3 + i * 0.11})`;
      ctx.fillText(levels[i], width * 0.5, y + s * 0.044);
    }
    for (let i = 0; i < 8; i++) {
      const p = fract(time * 0.04 + i * 0.12);
      circle(
        width * 0.5 + Math.sin(i * 2.2) * width * 0.2 * (1 - p),
        height * 0.81 - p * height * 0.63,
        2 + (i % 2),
        rgba([255, 240, 185], 0.5),
      );
    }
    footerLabel("EVIDENCIA CONVERGENTE, CONFIANZA REVISABLE", z);
  }
  function sceneAnnouncement(z, time) {
    const s = Math.min(width, height);
    drawGlobe(width * 0.5, height * 0.48, s * 0.2, z, time, true);
    for (let i = 0; i < 16; i++) {
      const a = (i / 16) * TAU + time * 0.008,
        rr = s * (0.29 + (i % 3) * 0.04),
        x = width * 0.5 + Math.cos(a) * rr,
        y = height * 0.48 + Math.sin(a) * rr * 0.62;
      drawPacket(
        x,
        y,
        s * 0.07,
      ...
      line(width * 0.5, height * 0.48, x, y, rgba(z.color, 0.08), 1);
    }
    footerLabel("LO OBSERVADO · LO INFERIDO · LO DESCONOCIDO", z);
  }
  function sceneCoordination(z, time) {
    const s = Math.min(width, height);
    drawGlobe(width * 0.5, height * 0.47, s * 0.18, z, time, true);
    const labels = ["CIENCIA", "GOB", "GLOBAL", "EMP", "COM", "SOC"];
    for (let i = 0; i < labels.length; i++) {
      const a = (i / labels.length) * TAU - time * 0.008,
        rr = s * 0.32,
        x = width * 0.5 + Math.cos(a) * rr,
        y = height * 0.47 + Math.sin(a) * rr * 0.58;
      drawPacket(
        x,
        y,
        s * 0.085,
        z,
        labels[i],
        i === Math.floor(pointerX * labels.length) ? 1 : 0.5,
      );
      line(x, y, width * 0.5, height * 0.47, rgba(z.color, 0.14), 1);
    }
    footerLabel("NINGUNA VOZ ÚNICA HABLA POR TODO EL PLANETA", z);
  }
  function sceneInterpretation(z, time) {
    const s = Math.min(width, height);
    drawWaveform(
      width * 0.08,
      height * 0.18,
    ...
    for (let i = 0; i < 7; i++) {
      ...
      line(
        ...
        x,
        height * 0.72,
        rgba(z.color, i === Math.floor(pointerX * 7) ? 0.55 : 0.1),
        i === Math.floor(pointerX * 7) ? 2 : 1,
      );
    }
    footerLabel("PRIMERO ESTRUCTURA · DESPUÉS SIGNIFICADO", z);
  }
  function sceneMath(z, time) {
    const s = Math.min(width, height);
    const cx = width * 0.5,
      cy = height * 0.46;
    ...
    for (let i = 0; i < 9; i++) {
      const a = (i / 9) * TAU + time * 0.01;
      circle(
        cx + Math.cos(a) * s * 0.29,
        cy + Math.sin(a) * s * 0.18,
        2 + (i % 3),
        rgba(i % 2 ? z.color : [255, 230, 150], 0.68),
      );
    }
    ctx.font = `900 ${s * 0.04}px system-ui`;
    ctx.textAlign = "center";
    ctx.fillStyle = "rgba(255,255,255,.58)";
    ctx.fillText("2 · 3 · 5 · 7 · 11", cx, height * 0.8);
    footerLabel("REFERENCIAS COMUNES · NO UN LENGUAJE COMPLETO", z);
  }
  function sceneMeaning(z, time) {
    const s = Math.min(width, height);
    ...
    for (let i = 0; i < 6; i++) {
      const y = height * (0.31 + i * 0.065);
      const shape = i % 3 === 0 ? "○" : i % 3 === 1 ? "△" : "↔";
      ctx.font = `900 ${s * 0.038}px system-ui`;
      ctx.textAlign = "center";
      ctx.fillStyle = rgba([255, 235, 180], 0.65);
      ctx.fillText(shape, rightX, y);
      ctx.setLineDash([4, 7]);
      line(
        width * 0.41,
        y,
        width * 0.68,
        y,
        rgba(z.color, i === Math.floor(pointerX * 6) ? 0.65 : 0.16),
        1.5,
      );
      ctx.setLineDash([]);
    }
    ...
    footerLabel("EJEMPLO → CONTRASTE → RELACIÓN → CONCEPTO", z);
  }
  function sceneLanguage(z, time) {
    ...
    drawGlyphField(
      ...
      z,
      time,
      Math.floor(pointerX * 6),
    );
    for (let i = 0; i < 5; i++) {
      const y = height * (0.25 + i * 0.095);
      ctx.strokeStyle = rgba(
        i % 2 ? z.color : [255, 235, 170],
        0.16 + i * 0.05,
      );
      ctx.strokeRect(width * 0.13, y, width * 0.74, height * 0.055);
    }
    drawWaveform(
      width * 0.17,
      height * 0.72,
    ...
    footerLabel("ESTRUCTURA SIN VOZ, ESCRITURA NI IMAGEN HUMANA", z);
  }
  function sceneCulture(z, time) {
    ...
    centers.forEach((p, i) =>
      drawCivilization(width * p[0], height * p[1], s * 0.15, z, time, i),
    );
    for (let i = 0; i < 3; i++) {
      ctx.setLineDash([4, 8]);
      line(
    ...
    drawPacket(width * 0.5, height * 0.78, s * 0.13, z, "CONTEXTO", 0.78);
    footerLabel("MISMA FÍSICA · HISTORIAS MENTALES DISTINTAS", z);
  }
  function sceneDecision(z, time) {
    const labels = [
      "SILENCIO",
      "RECIBIDO",
      "CIENCIA",
    ...
    footerLabel("RESPONDER, ESPERAR O CALLAR CAMBIA LA RELACIÓN", z);
  }
  function sceneDistance(z, time) {
    const s = Math.min(width, height),
      ax = width * 0.16,
      ay = height * 0.32,
      bx = width * 0.84,
      by = height * 0.62;
    drawStar(ax, ay, s * 0.045, [255, 220, 130], time);
    drawStar(bx, by, s * 0.045, [145, 205, 255], time);
    ctx.setLineDash([6, 10]);
    line(ax + s * 0.05, ay, bx - s * 0.05, by, rgba(z.color, 0.28), 1.5);
    ctx.setLineDash([]);
    for (let i = 0; i < 9; i++) {
      const p = fract(time * 0.025 + i / 9),
        x = mix(ax + s * 0.06, bx - s * 0.06, p),
        y = mix(ay, by, p);
      drawPacket(x, y, s * 0.045, z, i % 2 ? "·" : "1", 0.25 + 0.55 * (1 - p));
    }
    for (let i = 0; i < 5; i++) {
      ctx.strokeStyle = rgba(z.color, 0.1 + i * 0.035);
      ctx.beginPath();
      ctx.arc(width * 0.5, height * 0.47, s * (0.08 + i * 0.055), 0, TAU);
      ctx.stroke();
    }
    footerLabel("EL MENSAJE DESCRIBE EL PASADO DEL EMISOR", z);
  }
  function sceneFirstReply(z, time) {
    const s = Math.min(width, height);
    drawCivilization(width * 0.2, height * 0.55, s * 0.13, z, time, 0);
    drawCivilization(width * 0.8, height * 0.42, s * 0.13, z, -time, 1);
    ctx.setLineDash([5, 8]);
    line(
      width * 0.3,
      height * 0.53,
    ...
    for (let i = 0; i < 5; i++) {
      const p = fract(time * 0.04 + i * 0.2),
        x = mix(width * 0.3, width * 0.7, p),
        y = mix(height * 0.53, height * 0.44, p);
      drawPacket(
        x,
        y,
        s * 0.055,
    ...
    drawSignal(width * 0.8, height * 0.42, s * 0.08, z, time, 6);
    footerLabel("RECEPCIÓN DEMOSTRADA · COMPRENSIÓN TODAVÍA PARCIAL", z);
  }
  function sceneDialogue(z, time) {
    const s = Math.min(width, height);
    drawCivilization(width * 0.18, height * 0.5, s * 0.11, z, time, 0);
    drawCivilization(width * 0.82, height * 0.5, s * 0.11, z, -time, 2);
    for (let i = 0; i < 7; i++) {
      const y = height * (0.18 + i * 0.09),
        left = i % 2 === 0,
        x1 = left ? width * 0.28 : width * 0.72,
        x2 = left ? width * 0.72 : width * 0.28;
      ctx.setLineDash([5, 9]);
      line(x1, y, x2, y + height * 0.035, rgba(z.color, 0.14 + i * 0.035), 1.2);
      ctx.setLineDash([]);
      drawPacket(
        mix(x1, x2, fract(time * 0.025 + i * 0.13)),
        mix(y, y + height * 0.035, fract(time * 0.025 + i * 0.13)),
        s * 0.045,
        z,
        i < 2 ? "REF" : i < 4 ? "?" : "CTX",
        0.5,
    ...
    footerLabel("CADA TURNO PUEDE PERTENECER A OTRA GENERACIÓN", z);
  }
  function sceneKnowledge(z, time) {
    const s = Math.min(width, height);
    const labels = ["ASTRO", "FÍS", "QUÍM", "BIO", "TEC", "ARTE"];
    for (let i = 0; i < labels.length; i++) {
      const x = width * (0.1 + i * 0.16),
        y = height * (0.3 + 0.16 * (i % 2));
      drawPacket(x, y, s * 0.085, z, labels[i], 0.65);
      for (let k = 0; k < 3; k++) {
        const p = fract(time * 0.035 + i * 0.12 + k * 0.22);
        circle(
          mix(x, width * 0.5, p),
          mix(y, height * 0.74, p),
          2,
    ...
    drawPacket(width * 0.5, height * 0.76, s * 0.15, z, "VERIFICAR", 0.85);
    footerLabel("CONOCIMIENTO AJENO · AUTORIDAD NO AUTOMÁTICA", z);
  }
  function sceneMisunderstanding(z, time) {
    const s = Math.min(width, height);
    drawGlyphField(
      width * 0.08,
      height * 0.18,
    ...
    for (let i = 0; i < 6; i++) {
      const y = height * (0.24 + i * 0.065),
        offset = Math.sin(i * 2.4) * height * 0.08;
      ctx.setLineDash([3, 8]);
      line(
        width * 0.41,
        y,
        ...
        y + offset,
        rgba(
          i === Math.floor(pointerX * 6) ? [255, 150, 130] : z.color,
          i === Math.floor(pointerX * 6) ? 0.65 : 0.16,
        ),
        i === Math.floor(pointerX * 6) ? 2.5 : 1,
      );
      ctx.setLineDash([]);
    }
    ctx.font = `900 ${s * 0.12}px system-ui`;
    ctx.textAlign = "center";
    ctx.fillStyle = "rgba(255,255,255,.1)";
    ctx.fillText("?", width * 0.5, height * 0.78);
    footerLabel("DATOS ENTENDIDOS · INTENCIÓN TODAVÍA AMBIGUA", z);
  }
  function sceneTrust(z, time) {
    const s = Math.min(width, height);
    drawCivilization(width * 0.22, height * 0.46, s * 0.13, z, time, 0);
    drawCivilization(width * 0.78, height * 0.46, s * 0.13, z, -time, 1);
    for (let i = 0; i < 5; i++) {
      const y = height * (0.27 + i * 0.11),
        p = 0.5 + 0.45 * Math.sin(time * 0.25 + i);
      ctx.strokeStyle = rgba(
        i % 2 ? z.color : [255, 230, 160],
        0.16 + 0.23 * p,
      );
      ...
      circle(
        ...
        y,
        2 + (i % 2),
        rgba([255, 245, 200], 0.42 + 0.25 * p),
      );
    }
    drawPacket(width * 0.5, height * 0.79, s * 0.12, z, "CONSISTENCIA", 0.78);
    footerLabel("CONFIANZA PARCIAL · VERIFICACIÓN CONTINUA", z);
  }
  function sceneDiplomacy(z, time) {
    const s = Math.min(width, height);
    drawCivilization(width * 0.18, height * 0.48, s * 0.11, z, time, 0);
    drawCivilization(width * 0.82, height * 0.48, s * 0.11, z, -time, 2);
    drawGlobe(width * 0.5, height * 0.48, s * 0.12, z, time, false);
    const labels = [
      "QUIÉN",
      "PROTO",
      "INTERÉS",
    ...
    for (let i = 0; i < labels.length; i++) {
      const a = (i / labels.length) * TAU + time * 0.006,
        x = width * 0.5 + Math.cos(a) * s * 0.28,
        y = height * 0.48 + Math.sin(a) * s * 0.18;
      drawPacket(x, y, s * 0.065, z, labels[i], 0.5);
      line(x, y, width * 0.5, height * 0.48, rgba(z.color, 0.1), 1);
    }
    footerLabel("PROCEDIMIENTOS PARA CONVIVIR CON EL DESACUERDO", z);
  }
  function sceneCoexistence(z, time) {
    const s = Math.min(width, height);
    drawCivilization(width * 0.2, height * 0.48, s * 0.15, z, time, 0);
    drawCivilization(width * 0.8, height * 0.48, s * 0.15, z, -time, 2);
    for (let i = 0; i < 11; i++) {
      const y = height * (0.18 + i * 0.055),
        bend = Math.sin(i * 1.7) * height * 0.045;
      ctx.strokeStyle = rgba(
        i % 3 ? z.color : [255, 230, 175],
        0.08 + i * 0.025,
      );
      ...
      ctx.stroke();
      const p = fract(time * 0.02 + i * 0.09);
      circle(
        mix(width * 0.33, width * 0.67, p),
        y + bend * Math.sin(p * Math.PI),
        1.5 + (i % 2),
        rgba([255, 245, 205], 0.46),
      );
    }
    softHalo(width * 0.5, height * 0.48, s * 0.23, z.color, 0.08);
    ctx.font = `900 ${s * 0.13}px system-ui`;
    ctx.textAlign = "center";
    ctx.fillStyle = "rgba(255,255,255,.09)";
    ctx.fillText("≠", width * 0.5, height * 0.53);
    footerLabel("MÁS ALLÁ: PERSPECTIVAS CÓSMICAS", z);
  }
