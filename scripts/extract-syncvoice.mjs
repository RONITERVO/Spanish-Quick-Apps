...
const LOCALES = Object.freeze({
  "es-ES": Object.freeze({
    ...
    translationKey: null,
    direction:
      "Warm, patient educational narration; clear neutral Spanish, unhurried pacing, and gentle emphasis on key terms.",
  }),
  "en-US": Object.freeze({
    prefix: "sq-en",
    sourceDirectory: "en",
    translationKey: "en",
    direction:
      "Warm, patient educational narration for a school audience; clear neutral American English, unhurried pacing, and gentle emphasis on key terms.",
  }),
  "fi-FI": Object.freeze({
    prefix: "sq-fi",
    sourceDirectory: "fi",
    translationKey: "fi",
    direction:
      "Warm, patient educational narration for a school audience; clear neutral Finnish, unhurried pacing, and gentle emphasis on key terms.",
  }),
});
...
function externalIdFor(locale, sourceKey, appId = null) {
  ...
  const stableSourceKey =
    locale === "es-ES" ? sourceKey : `app-${appId}:${sourceKey}`;
...
async function loadTranslationModule(appId, locale) {
  ...
  if (!catalog || typeof catalog !== "object" || Array.isArray(catalog)) {
    throw new Error(
      `Catalog ${sourcePath} did not register app ${appId} / ${config.translationKey}`,
    );
  }
  return {
...
async function readCatalog(fileName) {
  ...
  if (!/^\d{2}$/.test(appId))
    throw new Error(`Unexpected catalog filename: ${fileName}`);
  ...
  const missingFinnish = sourceKeys.filter(
    (sourceKey) => !(sourceKey in fi.values),
  );
  const extraFinnish = fiSourceKeys.filter(
    (sourceKey) => !(sourceKey in en.values),
  );
  ...
  for (const sourceKey of sourceKeys) {
    if (!sourceKey.trim())
      throw new Error(`App ${appId}: empty Spanish source key`);
    if (!String(en.values[sourceKey]).trim())
      throw new Error(
        `App ${appId}: empty en-US value for ${JSON.stringify(sourceKey)}`,
      );
    if (!String(fi.values[sourceKey]).trim())
      throw new Error(
        `App ${appId}: empty fi-FI value for ${JSON.stringify(sourceKey)}`,
      );
  }
...
function catalogModule(appId, mappings) {
  const sortedMappings = Object.fromEntries(
    REQUIRED_LOCALES.map((locale) => [
      locale,
      Object.fromEntries(
        [...mappings[locale]].sort(([left], [right]) =>
...
async function main() {
  ...
  if (fileNames.length !== 25)
    throw new Error(`Expected 25 English catalogs, found ${fileNames.length}`);
  ...
  const spanishEntries = [...spanishProvenance.entries()].map(
    ([sourceKey, source]) => ({
  ...
  const manifest = {
    ...
    project: {
      ...
      description:
        "Committed Spanish, English, and Finnish learning narration shared by all 25 interactive apps.",
      defaultLocale: "es-ES",
  ...
  const spanishIdBySourceKey = new Map(
    spanishEntries.map((entry) => [entry.sourceKey, entry.externalId]),
  );
  await Promise.all(
    catalogs.map((catalog) => {
      const mappings = {
        "es-ES": catalog.sourceKeys.map((sourceKey) => [
          sourceKey,
          spanishIdBySourceKey.get(sourceKey),
        ]),
        "en-US": catalog.sourceKeys.map((sourceKey) => [
          sourceKey,
          externalIdFor("en-US", sourceKey, catalog.appId),
        ]),
        "fi-FI": catalog.sourceKeys.map((sourceKey) => [
          sourceKey,
          externalIdFor("fi-FI", sourceKey, catalog.appId),
        ]),
      };
      return atomicWrite(
        path.join(CATALOG_DIR, `${catalog.appId}.js`),
        catalogModule(catalog.appId, mappings),
      );
    }),
  );

  const sourceCount = catalogs.reduce(
    (sum, catalog) => sum + catalog.sourceKeys.length,
    0,
  );
  console.log(
