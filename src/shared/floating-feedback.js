...
export function mountFeedback() {
  ...
  function showFloatingText({ title, feature, metric, fact, x, y, color }) {
    ...
    floatingTexts.push({
      ...
      x: Math.max(
        maxWidth * 0.5 + 12,
        Math.min(width - maxWidth * 0.5 - 12, x),
      ),
  ...
  function feedbackJitterRadius() {
    return Math.min(42, Math.max(28, Math.min(width, height) * 0.07));
  }
  ...
  function drawFloatingTexts() {
    ...
    floatingTexts.forEach((text) => {
      ...
      if (text.feature) {
        const featureSize = fitFont(
          text.feature.toUpperCase(),
          Math.max(16, titleSize * 0.43),
          13,
          text.maxWidth,
          handFont,
        );
  ...
  const observer = new MutationObserver(() => {
    if (activePointer === null)
      showReadoutFeedback(false, null, shownSignaturesThisTouch.size > 0);
